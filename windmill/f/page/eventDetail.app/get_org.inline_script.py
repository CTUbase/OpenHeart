...
def main(id_evt: str):
    response = supabase.table("events").select("id_org").eq("id", id_evt).execute()
    id_org = response.data[0]["id_org"]
    response = supabase.table("organizations").select("*").eq("id", id_org).execute()
    data_org = response.data[0]
    id_account = response.data[0]["id_account"]
    response = supabase.table("accounts").select("*").eq("id", id_account).execute()
